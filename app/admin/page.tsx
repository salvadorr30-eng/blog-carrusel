  ...
  return (
    <div className="space-y-10">
        ...
        <div className="border border-ink/10 rounded-xl divide-y divide-ink/10">
          {books.length === 0 && (
            <p className="p-6 text-ink/60">
              Aún no hay libros. Empieza añadiendo uno.
            </p>
          )}
          {books.map((book) => (
            <div
              key={book.slug}
              className="flex items-center gap-4 p-4 hover:bg-ink/5"
            >
      ...
      <section className="bg-ink/5 rounded-xl p-6 text-sm text-ink/70 space-y-2">
        <p>
          <strong>¿Cómo funciona?</strong> El formulario crea un archivo{" "}
          <code>.mdx</code> en <code>content/libros/</code> y guarda la portada
          en <code>public/covers/</code>.
        </p>
        <p>
          Funciona con <code>npm run dev</code> en local. Cuando termines, sube
          los cambios a Git y Vercel los desplegará automáticamente.
        </p>
      </section>
    </div>
  );
